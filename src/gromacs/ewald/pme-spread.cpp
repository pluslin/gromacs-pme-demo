...
// input
    ...
    sprintf(filename, "input.recipbox");
    fp = fopen(filename, "w");
    fprintf(fp, "%f %f %f %f %f %f", rxx, ryx, ryy, rzx, rzy, rzz);
    fclose(fp);
#endif
...
// inter output
#ifdef WRITE_DATA
        sprintf(filename, "inter.idxptr");
        fp = fopen(filename, "a");
        fprintf(fp, "%d %d %d\n", idxptr[0], idxptr[1], idxptr[2]);
        fclose(fp);

        sprintf(filename, "inter.fptr");
        fp = fopen(filename, "a");
...
// input
#ifdef WRITE_DATA
    // TODO: write atom coordinate(three dimension)
    //       x y z\n
    char filename[100];
    FILE *fp;
    int tt;

    sprintf(filename, "input.atom");
    fp = fopen(filename, "w");
    for(tt = 0; tt < atc->n; tt++) {
        fprintf(fp, "%f %f %f\n", atc->x[tt][0], atc->x[tt][1], atc->x[tt][2]);
    }
    fclose(fp);

    // TODO: write atom coefficient
    sprintf(filename, "input.coefficient");
    fp = fopen(filename, "w");
    for(tt = 0; tt < atc->n; tt++) {
        fprintf(fp, "%f\n", atc->coefficient[tt]);
    }
